...
(deftest get-content-in-test
  (let [child-node (create-child ["some-value"])]
    (testing "Tag not found"
      (is (= nil
             (xml/get-content-in (create-xml-zipper ["some-value"]) [:not-exist]))))
    (testing "Nested tag not found"
      (is (= nil
             (xml/get-content-in (create-xml-zipper ["some-value"]) [:not-exist :also-not-exist]))))
    (testing "Has no content"
      (is (= nil
             (xml/get-content-in (create-xml-zipper nil) [:node]))))
    (testing "Content at root level"
      (is (= "some-value"
             (xml/get-content-in (create-xml-zipper ["some-value"]) [:node]))))
    (testing "Get grandchild content"
      (is (= "some-value"
             (xml/get-content-in (create-xml-zipper [child-node]) [:node :child]))))
    (testing "Get node with nested content"
      (is (= child-node
             (xml/get-content-in (create-xml-zipper [child-node]) [:node]))))
    (testing "get-in will only return the first child"
      (is (= "A"
             (xml/get-content-in (create-xml-zipper [(create-child ["A"])
                                             (create-child ["B"])])
                                 [:node :child]))))))
...
(deftest get-all-content-test
  (testing "No content found"
    (is (= []
           (xml/get-all-content [(create-child ["A"])]
                                :not-this-tag))))
  ...
  (testing "Get nested content"
    (is (= [{:attrs nil, :content ["A"], :tag :child}
           ...
           (xml/get-all-content [(create-child [(create-child ["A"])])
